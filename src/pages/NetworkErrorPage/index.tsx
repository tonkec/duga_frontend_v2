  ...
  return (
    <div className="gradient relative isolate flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 text-white">
      ...
      <section className="relative z-10 w-full max-w-2xl overflow-hidden rounded-3xl border border-white/20 bg-white/95 p-6 text-center text-gray-950 shadow-2xl shadow-blue-dark/20 backdrop-blur-md sm:p-8">
        ...
        <span className="mb-4 inline-flex rounded-full bg-red/10 px-4 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-red">
        ...
        <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full bg-blue px-6 py-3 text-sm font-semibold text-white shadow-md shadow-blue/20 transition-colors hover:bg-blue-dark"
          >
            <BiRefresh size={20} />
            Pokušaj ponovno
          </button>
        ...
        <div className="mt-8 grid gap-3 text-left sm:grid-cols-3">
          {['Provjeri internet vezu', 'Pričekaj kratko', 'Osvježi stranicu'].map((item) => (
            <div
              key={item}
              className="rounded-2xl bg-[#f7f9ff] px-4 py-3 text-sm font-semibold text-gray-700"
            >
              {item}
            </div>
          ))}
